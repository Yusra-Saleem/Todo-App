# Data Model: Secure Task Deletion and Completion Toggle
...
- **Name**: User
- **Description**: Represents an authenticated user who owns tasks and can delete or update completion status of their own tasks only
- **Fields**:
  - `id`: str (UUID, primary key, unique identifier)
...
### TaskUpdate DTO (New)
- **Name**: TaskUpdate
- **Description**: A Pydantic model for updating tasks with optional fields to support partial updates including completion status toggle
- **Fields**:
  ...
  - `is_completed`: Optional[bool] (optional, completion status - for toggle updates)

### TaskRead DTO (Existing)
- **Name**: TaskRead
...
- Toggle operation: `completed` flips to opposite value
- Deletion: Task record removed from database
- `updated_at` timestamp updated on any modification
...
- All fields are optional to support toggle-only updates
- `description` can be updated independently of other fields
...
### TaskUpdate Schema
- This Pydantic model is used for API requests to update tasks (specifically for toggling completion status)
...
- Verification: Query ensures both task ID and user ID match (atomically)
- Result: Task record removed from database
- Response: 204 No Content on success

### Secure Task Toggle
- Operation: PATCH /api/tasks/{id}/complete
- Verification: Query ensures both task ID and user ID match (atomically)
- Result: `completed` field toggled, `updated_at` updated
- Response: Updated Task object with TaskRead schema